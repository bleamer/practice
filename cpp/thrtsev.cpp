...
void searchNum(vector<long long> sqLst, vector<long long> Lst,vector<int> Lst7,vector<int> LstSq7){
	for(vector<long long>::size_type x = 0; x != numSqList.size(); ++x) {
		for(vector<long long>::size_type y = 0; y != numSqList.size(); ++y) {
			if(binary_search(Lst.begin(), Lst.end(), sqLst[x]+sqLst[y]))
			{
...
int main(int argc, char const *argv[])
{
	...
	cin >> digitCount;
	genNum(0,digitCount);
	searchNum(numSqList, numList,num7CntList,numSq7CntList);
